...
const Login = () => {
 ...
 try {
    ...
    const {data} = await axios.post(backendUrl + '/api/user/register',{name,password,email,phone})
    if (data.success) {
...
navigate('/')
  ...
  state === "Sign Up" && (
    <>
      ...
      <div className='w-full'>
        <p>Phone No</p>
        <input className='border border-zinc-300 rounded w-full p-2 mt-1' type="text" onChange={(e) => setPhone(e.target.value)} value={phone} />
      </div>
    </>
  )
}
